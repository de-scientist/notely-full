  ...
  try {
    ...
    if (!title || !synopsis || !content || !categoryId) {
      return res
        .status(400)
        .json({
          message: "Title, synopsis, content, and categoryId are required.",
        });
    }
...
router.patch(
  "/:id",
  async (req: Request<{ id: string }, {}, EntryUpdateData>, res, next) => {
    try {
      const userId = req.user!.id;
      const { id } = req.params;
      // 🎯 FIX APPLIED: TypeScript now recognizes the shape of req.body
      const { title, synopsis, content, categoryId, pinned, isPublic } =
        req.body;
      ...
      if (categoryId !== undefined) {
        const valid = await prisma.category.findUnique({
          where: { id: categoryId },
        });
        if (!valid)
          return res
            .status(404)
            .json({ message: "Invalid categoryId provided." });
