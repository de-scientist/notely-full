...
const PRIMARY_HOVER_CLASS =
  "hover:text-fuchsia-600 dark:hover:text-fuchsia-500";
...
const COMPLEMENTARY_GRADIENT_CLASS =
  "bg-gradient-to-r from-emerald-500 to-emerald-700 hover:from-emerald-600 hover:to-emerald-800 text-white shadow-md shadow-emerald-500/30 transition-all duration-300 transform hover:scale-[1.03]";
...
function UserNav() {
  ...
  const avatarSrc = user?.avatar ?? undefined; // Use a function to safely call logout and handle navigation/UI state
  ...
  const handleLogout = () => {
    ...
    clear(); // No need to navigate, as ProtectedRoute will handle redirect.
  };
      ...
      <DropdownMenuTrigger asChild>
        ...
        <div className="flex items-center gap-3 cursor-pointer group hover:opacity-90 transition-opacity">
          ...
          <span className="hidden text-right lg:inline">
            ...
                                   {" "}
            <span
              className={`block font-semibold text-gray-800 dark:text-gray-100 group-hover:${PRIMARY_TEXT_CLASS}`}
            >
                                          {user.firstName}                     
               {" "}
            </span>
          ...
          >
            ...
                                   {" "}
            <AvatarFallback
              className={`${PRIMARY_BG_CLASS} text-primary-foreground font-bold text-xs`}
            >
                                         {" "}
