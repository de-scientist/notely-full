 ...
 */
function getDaysUntilPermanentDelete(updatedAt: string): {
  deletedOn: string;
    ...
    return (
      <div className="mt-16 flex flex-col items-center justify-center text-center space-y-6 p-8 border-2 border-dashed border-fuchsia-300 dark:border-fuchsia-800 rounded-xl bg-gray-50 dark:bg-gray-900/50">
        ...
        <p className="text-gray-600 dark:text-gray-400 max-w-lg">
          Your trash bin is currently clean. Any notes you delete will appear
          here before their final deletion.
        </p>
    ...
    <div className="space-y-8 p-4 sm:p-6 lg:p-8">
      <div className="flex justify-between items-center pb-4 border-b dark:border-gray-700">
        <div className="space-y-1">
          <h1 className="text-4xl font-extrabold dark:text-white flex items-center gap-3">
            <Trash2 className={`h-8 w-8 ${PRIMARY_COLOR_CLASS}`} /> Trash (
            {entries.length} items)
          </h1>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Review and restore notes. Items are permanently removed 30 days
            after deletion.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {entries.map((entry) => {
          ...
          const { deletedOn, daysRemaining } = getDaysUntilPermanentDelete(
            entry.updatedAt,
          );
          ...
          return (
            ...
            >
              ...
              <CardHeader className="pb-2">
                <CardTitle className="text-xl font-semibold dark:text-white line-clamp-1">
                  {entry.title}
                </CardTitle>
              ...
              <CardContent className="pt-0 pb-4 flex-1">
                ...
                <div className="text-sm border-t pt-3 dark:border-gray-700">
                  ...
                  <p
                    className={`font-semibold mt-1 flex items-center gap-1 ${daysClass}`}
                  >
      ...
      <div className="p-4 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 rounded-lg text-sm text-red-700 dark:text-red-400 flex items-center gap-2 mt-8">
        ...
        <span className="font-medium">
          **Warning:** Permanently deleting an item removes it forever and
          **cannot be undone**. Items are automatically removed after 30 days.
        </span>
      </div>
    </div>
  );
